...
export default function AdminProductsPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: (payload) =>
      editId ? adminAPI.updateProduct(editId, payload) : adminAPI.createProduct(payload),
    onSuccess: () => {
      ...
      {showForm && (
        <div className="bg-white border border-gray-100 rounded-2xl p-6 mb-5">
          ...
          <form onSubmit={handleSubmit}>
            ...
            {/* Image Upload Zone */}
            <div className="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center mb-5 cursor-pointer hover:border-accent hover:bg-red-50 transition-all">
              <p className="text-2xl mb-1">📁</p>
              <p className="text-sm font-medium text-gray-600">Click or drag to upload product images</p>
              <p className="text-xs text-gray-400 mt-1">PNG, JPG, WEBP up to 10MB (max 5 images)</p>
                  ...
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center text-xl flex-shrink-0">
                        {p.thumbnail ? <img src={p.thumbnail} alt="" className="w-full h-full object-cover rounded-lg" /> : "📦"}
                      </div>
                      <div>
